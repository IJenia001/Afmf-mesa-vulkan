...
# Сначала обновляем пакеты и устанавливаем базовые зависимости
echo "Обновление системы и установка базовых пакетов..."
apt update
apt install -y python3-pip python3-full python3-venv software-properties-common

# Пытаемся установить meson из системных пакетов
apt install -y meson || true

# Проверяем наличие meson и устанавливаем при необходимости
if ! command -v meson &> /dev/null; then
    echo "Установка Meson через pip с --break-system-packages..."
    pip3 install --break-system-packages meson
    # Добавляем путь к pip пакетам в PATH для текущей сессии
    export PATH="$PATH:/home/$SUDO_USER/.local/bin:/root/.local/bin"
fi
...
if ! dpkg --compare-versions "$CURRENT_MESON_VERSION" ge "$MIN_MESON_VERSION"; then
    echo "Обновление Meson до версии >= $MIN_MESON_VERSION (текущая: $CURRENT_MESON_VERSION)..."
    pip3 install --break-system-packages --upgrade meson
fi

# Установка зависимостей
echo "Установка зависимостей для сборки..."
apt install -y build-essential ninja-build pkg-config \
python3-mako python3-yaml bison flex byacc glslang-tools \
...
cmake git wget curl unzip gawk libxml2-dev \
libopenblas-dev ocl-icd-opencl-dev vulkan-tools \
...
if [ ! -f "/usr/local/lib/libtensorflowlite.so" ]; then
    ...
    wget -q --show-progress https://github.com/tensorflow/tensorflow/archive/refs/tags/v${TFLITE_VERSION}.tar.gz -O tensorflow.tar.gz
...
if [ ! -f "mesa-25.0.3.tar.xz" ]; then
    echo "Загрузка Mesa..."
    wget -q --show-progress https://archive.mesa3d.org//mesa-25.0.3.tar.xz
fi
...
# Убедимся, что meson доступен в PATH
export PATH="$PATH:/home/$SUDO_USER/.local/bin:/root/.local/bin"
...
if [ -f /usr/local/lib/dri/ai_dri.so ]; then
    ...
    echo "Проверка всех DRI драйверов:"
    ls -la /usr/local/lib/dri/ 2>/dev/null || echo "Каталог DRI не найден"
fi
...
echo "- Версия TensorFlow Lite: установлена в /usr/local/lib/"